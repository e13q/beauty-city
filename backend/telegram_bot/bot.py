...
from datacenter.models import (
    Client, Salon, Service, Appointment, Order, Specialist
)
...
def list_salons(update: Update, context: CallbackContext):
    ...
    keyboard = [[InlineKeyboardButton(
        f"{salon.title} - {salon.address}", callback_data=f'salon_{salon.id}'
        ) for salon in salons]]
...
def choose_date(update: Update, context: CallbackContext):
    ...
    query.edit_message_text(
        text="Выберите время:", reply_markup=time_keyboard()
    )
    return CHOOSE_TIME
...
def time_keyboard():
    ...
    for i in range(18):  # 18 слотов по 30 минут с 9:00 до 18:00
        time_slot = (
            datetime.combine(datetime.today(), base_time) + timedelta(minutes=30 * i)
        ).time()
        keyboard.append([InlineKeyboardButton(
...
def choose_time(update: Update, context: CallbackContext):
    ...
    selected_datetime = datetime.strptime(
        f"{context.user_data['selected_date']} {selected_time}",
        "%Y-%m-%d %H:%M"
    )
    ...
    appointments = Appointment.objects.filter(
        ...
        start_at=selected_datetime.time()
    )

    if appointments.exists():
        query.edit_message_text(
            f'Выбранное время {selected_time} занято. Пожалуйста, выберите другое время.',
            reply_markup=time_keyboard()
        )
        return CHOOSE_TIME

    query.edit_message_text(
        f'Вы выбрали время с {selected_time}. Подтвердите заказ:',
...
def confirm_order_keyboard():
    keyboard = [
        [InlineKeyboardButton(
            'Оформить заказ', callback_data='confirm_order')],
        [InlineKeyboardButton(
            'Назад', callback_data='time_back')]
    ]
    return InlineKeyboardMarkup(keyboard)
...
def confirm_order(update: Update, context: CallbackContext):
    ...
    try:
        ...
        query.edit_message_text(
            f'Записано на {selected_datetime}')
    except Exception as e:
        query.edit_message_text(
            f'Ошибка при оформлении заказа: {str(e)}. Попробуй еще раз.')
        return ConversationHandler.END
...
def main():
    ...
    updater.dispatcher.add_handler(
        CallbackQueryHandler(back_to_date, pattern='^date_back$')
    )
    updater.dispatcher.add_handler(
        CallbackQueryHandler(back_to_time, pattern='^time_back$')
    )
    updater.dispatcher.add_handler(
