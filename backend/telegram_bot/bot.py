...
def list_salons(update: Update, context: CallbackContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(f"{salon.title} - {salon.address}",
                                 callback_data=f'salon_{salon.id}') for salon in salons
                                 ]
                                 ]
...
def list_services(update: Update, context: CallbackContext):
    ...
    keyboard = [
        [
            InlineKeyboardButton(f"{service.title} - {service.price}", 
...
def contact_admin(update: Update, context: CallbackContext):
    query = update.callback_query
    query.answer()
    admin_contact_info = "связь с администратором по телефону: +7 123 456 78 90 или по email: admin@example.com выбирай сам или сама что тебе удобно"
    query.edit_message_text(text=admin_contact_info, reply_markup=back_to_main_menu_keyboard())


def back_to_main_menu_keyboard():
    keyboard = [
        [InlineKeyboardButton('Назаl', callback_data='main_menu')]
    ]
    return InlineKeyboardMarkup(keyboard)
...
def date_keyboard():
    ...
    for i in range(7):
        ...
        keyboard.append([
            InlineKeyboardButton(day.strftime('%d-%m-%Y'), 
...
def time_keyboard():
    ...
    for i in range(10):
        ...
        keyboard.append([
            InlineKeyboardButton(time_slot.strftime('%H:%M'), 
...
def choose_time(update: Update, context: CallbackContext):
    ...
    query.edit_message_text(f'Ты выбрал время с {selected_time}. потверди правильно:', reply_markup=confirm_order_keyboard())
...
def back_to_service(update: Update, context: CallbackContext):
    ...
    services = Service.objects.all()
    keyboard = [
        [InlineKeyboardButton(f"{service.title} - {service.price} руб.", 
                            callback_data=f'service_{service.id}') 
                            for service in services]
    ]
